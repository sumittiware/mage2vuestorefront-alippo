...
class ProductAdapter extends AbstractMagentoAdapter {
  ...
  getFilterQuery(context) {
    ...
    if (context.skus) {
      // pul individual products
      if (!Array.isArray(context.skus)) context.skus = new Array(context.skus);

      query +=
        "searchCriteria[filter_groups][0][filters][0][field]=sku&" +
        "searchCriteria[filter_groups][0][filters][0][value]=" +
        encodeURIComponent(context.skus.join(",")) +
        "&" +
        "searchCriteria[filter_groups][0][filters][0][condition_type]=in";
    } else if (
      context.updated_after &&
      typeof context.updated_after == "object"
    ) {
      query +=
        "searchCriteria[filter_groups][0][filters][0][field]=updated_at&" +
        "searchCriteria[filter_groups][0][filters][0][value]=" +
        encodeURIComponent(moment(context.updated_after).utc().format()) +
        "&" +
  ...
  getSourceData(context) {
    ...
    const retryHandler = (context, err, reject) => {
      ...
      if (err == null || context.retry_count < HTTP_RETRIES) {
        ...
        if (
          this.config.product &&
          this.config.product.synchronizeCatalogSpecialPrices
        ) {
            ...
            this.getProductSourceData(context)
              .then((result) => {
                ...
                this.api.products
                  .renderList(
                    query,
                    this.config.magento.storeId,
                    this.config.magento.currencyCode
                  )
                  .then((renderedProducts) => {
                    context.renderedProducts = renderedProducts;
                    for (let product of result.items) {
                      const productAdditionalInfo = renderedProducts.items.find(
                        (p) => p.id === product.id
                      );
  ...
  getProductSourceData(context) {
    ...
    let searchCriteria =
      "&searchCriteria[currentPage]=%d&searchCriteria[pageSize]=%d";

    if (
      this.config.product &&
      JSON.parse(this.config.product.excludeDisabledProducts)
    ) {
      searchCriteria +=
    ...
    if (context.for_total_count) {
      // get total counts
      return this.api.products
        .list(util.format(searchCriteria, 1, 1))
        .catch((err) => {
          throw new Error(err);
        });
      ...
      logger.debug(
        `Using specific paging options from adapter context: ${context.page} / ${context.page_size}`
      );

      return this.api.products
        .list(
          util.format(searchCriteria, context.page, context.page_size) +
            (query ? "&" + query : "")
        )
        .catch((err) => {
          throw new Error(err);
      ...
      logger.debug(
        util.format(searchCriteria, this.page, this.page_size) +
          (query ? "&" + query : "")
      );
      return this.api.products
        .list(
  ...
  processAttributes(customAttributes, configurableOptions) {
    const loadFromCache = (key) =>
      new Promise((resolve) =>
        this.cache.get(key, (err, serializedAtr) =>
          resolve(JSON.parse(serializedAtr))
        )
      );
    ...
    const selectFields = (res) =>
      res.map((o) => {
        const attributeOptionValues = findOptionValues(o);
        const options = o.options.filter((opt) =>
          attributeOptionValues.includes(parseInt(opt.value))
        );
    ...
    const attributeCodes = customAttributes.map(
      (obj) =>
        new Promise((resolve) => {
          const key = util.format(
            CacheKeys.CACHE_KEY_ATTRIBUTE,
            obj.attribute_code
          );
    ...
    const attributeIds = configurableOptions.map(
      (obj) =>
        new Promise((resolve) => {
          const key = util.format(
            CacheKeys.CACHE_KEY_ATTRIBUTE,
    ...
    return Promise.all([...attributeCodes, ...attributeIds]).then(selectFields);
  }
  ...
  preProcessItem(item) {
    ...
    return new Promise((done, reject) => {
        ...
        subSyncPromises.push(() => {
          return this.api.stockItems.list(item.sku).then((result) => {
            ...
            if (this.config.magento.msi.enabled) {
              return this.api.stockItems
                .getSalableQty(item.sku, this.config.magento.msi.stockId)
                .then((salableQty) => {
                ...
                .then((item) => {
                  return this.api.stockItems
                    .isSalable(item.sku, this.config.magento.msi.stockId)
                    .then((isSalable) => {
                      item.stock.is_in_stock = isSalable;

                      const key = util.format(
                        CacheKeys.CACHE_KEY_STOCKITEM,
                        item.id
                      );
      ...
      if (this.custom_sync) {
        logger.info(
          `Product sub-stage 3: Getting product custom options ${item.sku}`
        );
        subSyncPromises.push(() => {
          return this.api.customOptions.list(item.sku).then((result) => {
            if (result && result.length > 0) {
              ...
              logger.info(
                `Found custom options for ${item.sku}: ${result.length}`
              );
            }
            return item;
          });
      ...
      if (this.custom_sync && item.type_id == "bundle") {
        logger.info(
          `Product sub-stage 4: Getting bundle custom options ${item.sku}`
        );
        subSyncPromises.push(() => {
          return this.api.bundleOptions.list(item.sku).then((result) => {
            if (result && result.length > 0) {
              ...
              logger.info(
                `Found bundle options for ${item.sku}: ${result.length}`
              );
            }
            return item;
          });
            ...
            return this.api.productLinks.types().then((result) => {
              if (result && result.length > 0) {
                ...
                for (const linkType of result) {
                  logger.info(
                    `Getting the product links ${item.sku}: ${linkType.name}`
                  );
                  subPromises.push(
                    this.api.productLinks
                      .list(item.sku, linkType.name)
                      .then((links) => {
                        if (links && links.length > 0) {
                          item.links[linkType.name] = links.map((r) => {
                            return {
                              sku: r.linked_product_sku,
                              pos: r.position,
                            };
                          });
                          logger.info(
                            `Found related products for ${item.sku}: ${
                              item.links[linkType.name]
                            }`
                          );
                        }
                        return item;
          ...
          return new Promise((opResolve, opReject) => {
            ...
            this.db
              .getDocuments("product", {
                query: { match: { "configurable_children.sku": item.sku } },
              })
              .then((docs) => {
                if (docs && docs.length > 0) {
                  ...
                  docs.map((parentProduct) => {
                    // schedule for update
                    queue
                      .createJob("product", {
                        skus: [parentProduct.sku],
                        adapter: "magento",
                      })
                      .save();
                    logger.info(
                      "Parent product update scheduled (make sure `cli.js productsworker` queue is running)",
                      parentProduct.sku
                    );
                  });
                  opResolve(item);
      ...
      if (this.configurable_sync && item.type_id == "configurable") {
        logger.info(
          `Product sub-stage 6: Getting product options for ${item.sku}`
        );

        // q.push(() => {
        subSyncPromises.push(() => {
          return new Promise((opResolve, opReject) => {
            this.api.configurableChildren
              .list(item.sku)
              .then((result) => {
                ...
                for (let prOption of result) {
                  ...
                  if (
                    context.renderedProducts &&
                    context.renderedProducts.items.length
                  ) {
                    const renderedProducts = context.renderedProducts;
                    const subProductAdditionalInfo =
                      renderedProducts.items.find((p) => p.id === confChild.id);

                    if (
                      subProductAdditionalInfo &&
                      subProductAdditionalInfo.price_info
                    ) {
                      delete subProductAdditionalInfo.price_info
                        .formatted_prices;
                      delete subProductAdditionalInfo.price_info
                        .extension_attributes;
                      // delete subProductAdditionalInfo.price_info.special_price // always empty :-(
                      confChild = Object.assign(
                        confChild,
                        subProductAdditionalInfo.price_info
                      );
                  ...
                  if (item.price == 0)
                    // if price is zero fix it with first children
                ...
                if (
                  ...
                  for (const attrToExpand of config.product
                    .expandConfigurableFilters) {
                        ...
                        new Promise((resolve, reject) => {
                          ...
                          this.cache.get(atrKey, (err, serializedAtr) => {
                            ...
                            if (atr != null) {
                              ...
                              logger.info(
                                `Product options for ${atr.attribute_code} for ${item.sku} set`
                              );
                              item[atr.attribute_code + "_options"] =
                                option.values.map((el) => {
      ...
      subSyncPromises.push(() => {
        return new Promise((resolve, reject) => {
          logger.info(
            `Product sub-stage 6: Getting product categories for ${item.sku}`
          );

          const key = util.format(
            CacheKeys.CACHE_KEY_PRODUCT_CATEGORIES,
            item.sku
          ); // store under SKU of the product the categories assigned
          ...
          if (this.category_sync) {
            ...
            const catBinder = (categories) => {
              ...
              for (let catId of categories) {
                catPromises.push(
                  new Promise((resolve, reject) => {
                    let cat = this.cache.get(
                      util.format(CacheKeys.CACHE_KEY_CATEGORY, catId),
                      (err, serializedCat) => {
              ...
              Promise.all(catPromises).then((values) => {
                if (this.category_sync)
                  // TODO: refactor the code above to not get cache categorylinks when no category_sync required
            ...
            if (
              ...
              const catIdsArray = item.category_ids.map((item) => {
                return parseInt(item);
              });
              logger.info(
                `Using category_ids binding for ${item.sku}: ${catIdsArray}`
              );
  ...
  computeVideoData(mediaItem) {
    ...
    if (
      mediaItem.extension_attributes &&
      mediaItem.extension_attributes.video_content
    ) {
      let videoId = null,
        type = null,
        youtubeRegex =
      ...
      if (
        mediaItem.extension_attributes.video_content.video_url.match(
          youtubeRegex
        )
      ) {
        videoId = RegExp.$1;
        ...
        mediaItem.extension_attributes.video_content.video_url.match(vimeoRegex)
      ) {
        videoId = RegExp.$3;
        type = "vimeo";
      }
