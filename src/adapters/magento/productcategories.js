...
class ProductcategoriesAdapter extends AbstractMagentoAdapter {
  ...
  _storeCatLinks(item, result) {
    ...
    switch (this.mode) {
      ...
      default: {
        // update in redis = ProductCategoriesModes.SEPARATE_REDIS
        logger.debug(
          `Storing category assigments in REDIS cache for ${item.id}`
        );

        for (let catLink of result) {
          const key = util.format(
            CacheKeys.CACHE_KEY_PRODUCT_CATEGORIES_TEMPORARY,
            catLink.sku
