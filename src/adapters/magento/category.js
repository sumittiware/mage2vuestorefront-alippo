...
class CategoryAdapter extends AbstractMagentoAdapter {
  ...
  _addSingleCategoryData(item, result) {
    item = Object.assign(
      item,
      _normalizeExtendedData(result, this.generateUniqueUrlKeys, this.config)
    );
  }
  ...
  _extendSingleCategory(rootId, catToExtend) {
    ...
    return this.api.categories
      .getSingle(catToExtend.id)
      .then(function (result) {
        Object.assign(
          catToExtend,
          _normalizeExtendedData(result, generateUniqueUrlKeys, config)
        );
        logger.info(
          `Subcategory data extended for ${rootId}, children object ${catToExtend.id}`
        );
      })
      .catch(function (err) {
        logger.error(err);
  ...
  _extendChildrenCategories(rootId, children, result, subpromises) {
    for (const child of children) {
      if (Array.isArray(child.children_data)) {
        this._extendChildrenCategories(
          rootId,
          child.children_data,
          result,
          subpromises
      ...
      if (this.extendedCategories) {
        this.api.categories
          ...
          .then((result) => {
            ...
            logger.debug(
              `Storing extended category data to cache under: ${key}`
            );
  ...
  normalizeDocumentFormat(item) {
    if (
      ...
      request(
        this.config.vuestorefront.invalidateCacheUrl + "C" + item.id,
        {},
        (err, res, body) => {
          if (err) {
