...
class AbstractAdapter {
  validateConfig(config) {
    if (!config["db"]["url"]) throw Error("db.url must be set up in config");
  }
  ...
  cleanUp(transaction_key) {
    this.db.connect(() => {
      ...
      this.db.cleanupByTransactionkey(
    ...
    items.map((item) => {
      this.preProcessItem(item)
        .then((item) => {
          ...
          logger.info(
            `Importing ${index} of ${count} - ${this.getLabel(
              item
            )} with tsk = ${item.tsk}`
          );
          ...
          if (this.tasks_count == 0 && !this.use_paging) {
            ...
            if (index == count - 1) {
              // page done!
              logger.debug(
                `--L:${level} Level done! Current page: ${this.page} of ${this.page_count}`
              );
              if (parseInt(level) == 0) {
                if (this.use_paging && !this.isFederated()) {
                  //TODO: paging should be refactored using queueing
